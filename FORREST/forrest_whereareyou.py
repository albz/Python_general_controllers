...
class run_status:
	...
	def count_run_allstatus(self,program):
		nsim=0
		for id in self.identifier:
			if self.program[id] == program:
				nsim += 1
		return nsim

	def print_status(self,program,status):
		if( self.count_run_allstatus(program) > 0):
			print program,' : ', status , ' : >>>', self.count_run(program,status)
			for id in self.identifier:
				if self.program[id] == program and self.status[id] == status and program == 'dued':
					print self.path[id], '--- lastest sim time: %2.4f (ns) --- dt=%2.4e' % (self.time[id],self.dt[id])
				if self.program[id] == program and self.status[id] == status and program == 'architect':
					print self.path[id], '--- lastest sim distance: %2.1f (um)' % (self.time[id])
...
#--- *** ---#
def find_running_time(path,program):
	if program == 'dued.nml':
		try:
...
for root_dir, sub_dirs, files in os.walk(path):
	...
	if 'dued.nml' in files:
		time = find_running_time(root_dir,'dued')
	...
	if 'architect.nml' in files:
		time = find_running_time(root_dir,'architect')
		dt   = find_last_dt(root_dir)
		flag = get_flag(root_dir)
